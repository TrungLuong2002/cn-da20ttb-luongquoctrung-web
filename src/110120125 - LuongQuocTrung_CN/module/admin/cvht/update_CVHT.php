...
if (isset($_POST['btn-suaCVHT'])) {
    ...
    if ($tengv == "") {
        $error['tengv'] = "Không được để trống tên giảng viên";
    }
    if ($email == "") {
    ...
    if ($quyetdinh == "") {
        $error['quyetdinh'] = "Không được để trống quyết định";
    }
    if (empty($error)) {
...
<form id="wp-frmUpdateCVHT" action="" method="post">
    <h3>Chỉnh sửa thông tin cố vấn</h3>
    ...
    <p class="err-del">
        <?php if (isset($error['tengv'])) echo $error['tengv']; ?>
    </p>
